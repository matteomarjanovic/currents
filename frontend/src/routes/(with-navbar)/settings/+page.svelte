...
<div class="mx-auto flex w-full max-w-2xl flex-col gap-8">
	<header>
		...
		<p class="text-sm text-muted-foreground">
			Saved locally in this browser. <b>Hide</b> filters matching saves out of every feed entirely.
			<b>Blur</b> shows a click-to-reveal warning. <b>Show</b> renders normally.
		</p>
	</header>
		...
		<div class="flex flex-col gap-3">
			{#each ADULT_CATEGORIES as cat (cat.key)}
				<div class="flex flex-col gap-3 rounded-lg border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
					...
					<div class="inline-flex shrink-0 rounded-md border border-border p-0.5 self-start sm:self-auto">
						{#each ADULT_OPTIONS as opt (opt.val)}
							{@const active = modPrefs[cat.key] === opt.val}
							<button
								type="button"
								onclick={() => setAdult(cat.key, opt.val)}
								class="rounded px-3 py-1.5 text-xs font-medium transition-colors {active
