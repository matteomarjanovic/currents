...
- **Starred collections** are stored in the `starred_collection` table (viewer_did + collection_uri PK). The `getActorCollections` and `getSaves` endpoints hydrate viewer state (`starred`) when the request is authenticated.
